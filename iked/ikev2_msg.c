...
ikev2_msg_encrypt(struct iked *env, struct iked_sa *sa, struct ibuf *src)
{
	...
	log_debug("%s: decrypted length %zd", __func__, len);
	print_hex(buf, 0, len);
	...
	log_debug("%s: padded length %zd", __func__, ibuf_size(src));
	...
	log_debug("%s: length %zd, padding %d, output length %zd",
...
ikev2_msg_integr(struct iked *env, struct iked_sa *sa, struct ibuf *src)
{
	...
	log_debug("%s: message length %zd", __func__, ibuf_size(src));
	print_hex(ibuf_data(src), 0, ibuf_size(src));
	...
	log_debug("%s: integrity checksum length %zd", __func__,
	    integrlen);
...
ikev2_msg_auth(struct iked *env, struct iked_sa *sa, int response)
{
	...
	log_debug("%s: %s auth data length %zd",
